...
SELECT id, user_id, title, type, description, created_at FROM categories WHERE user_id = $1 AND type = $2 AND LOWER(title) LIKE CONCAT('%', LOWER($3::text), '%') AND LOWER(description) LIKE CONCAT('%', LOWER($4::text), '%')
`
